...
|`$#`|引数の数を示す|
|`$数字`|指定した数字の引数を示す|
|`$?`|実行されたコマンドの終了ステータス値が格納されている。<br>通常0が正常終了|
|`$$`|実行中のシェルスクリプトのプロセスID(PID)が格納される|
|`$0`|実行中のシェルスクリプトのファイル名が格納される|
|`$*`|スクリプトに渡された引数全てを示す。<br>引数ごとの区切り文字はIFSに影響される|
|`$@`|上記と同上<br>ただし、IFSの影響は受けずスペース区切りで全引数を保持する|
